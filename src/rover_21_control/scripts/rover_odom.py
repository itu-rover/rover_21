...
class Localization(object):
	def __init__(self):
		...
		self.surrounding_of_wheel_new = 0.125 * math.pi * 2 #0.135 for rover21 // 0.155 for rover20 // 0.165 for husky #surrounding of the wheels in meters
		self.surrounding_of_wheel_old = 0.125 * math.pi * 2
	...
	def imu_cb(self, data):
		...
		[self.curr_roll, self.curr_pitch, self.curr_yaw] = self.euler_from_quaternion(data.orientation.x, data.orientation.y, data.orientation.z, data.orientation.w) #Convert quaternion to euler angles
		print(self.curr_yaw)
	...
	def controller(self):
		...
		while not rospy.is_shutdown():
			...
			self.q = self.quaternion_from_euler(0, 0, (self.curr_yaw - self.init_yaw)) #Rotation of the rover in quaternions
			self.odom = Odometry()
