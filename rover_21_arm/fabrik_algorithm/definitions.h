...
/* Bu metod bir değişkenli constructor dır. coordinate sınıfına ait yeni bir obje oluşturulurken 3 adet parametre verilirse,
otomatik olarak çalışır. Verilen parametreleri objenin X,Y ve Z verilerine eşitler.*/
...
double coordinate::calculate_dist(coordinate second_cord){
	...
	return sqrt(x_dist*x_dist + y_dist*y_dist + z_dist*z_dist);						// İki kere pisagor teoremi uygulanarak uzaklık bulunmuş olur. 
}
...
çağıran noktanın X,Y ve Z koordinatları ile parametre noktanın X,Y ve koordinatlarını toplayıp, yeni bir koordinat döndürmektir.*/
// to_be_added: Toplamada kullanılacak nokta

coordinate coordinate::operator+(coordinate to_be_added){

	double new_x = (this->get_x()) + to_be_added.get_x();							// 'this' pointer ı her zaman metodu çağıran objenin adresini tutar.
	double new_y = (this->get_y()) + to_be_added.get_y();							// Bu durumda this->get_x(), metodu çağıran objenin X koordinatını döndürecektir.
...
double angle_of_vectors(coordinate one, coordinate two){
	...
	double angle = (acos(dot_product(one,two)/(magnitude_one*magnitude_two))*180)/PI;			// Formül sonucunda vektörler arasındaki açı bulunur. Açı dereceye çevrilir.
...
/*Bu fonksiyon, eklem konumlarına göre yönlü açı bulur. Bir eklem açısını oluşturan 3 eklem vardır. Fonsiyon, önce 3 eklemin
oluşturduğu üçgenden kosinüs teoremi ile ortanca eklemin açısını bulur ancak bu açı, yanlış açı olabilir. Çünkü bütün açılar
için ortak yön olarak 0 pozisyonunda X-Y düzlemine bakan yön seçildi ve bulunan açı bu yöndeki açı değil ise, 360'dan çıkarılması
gerekebilir. Bu durumda yönlü açı bulmak için vektörel çarpım kullanılabilir. 3 eklemden orta eklemden bir önceki ekleme giden vektör
...
double find_angle(coordinate center, coordinate before, coordinate next, double joint_1_angle){
	...
	double my_angle = cosinus_theorem(center, before, next);						// Kosinüs teoremi ile ortanca eklemin açısı hesaplanır (Kontrol edilmedi. Yanlış olabilir.).																				

	coordinate new_before = coordinate(before.get_x()-center.get_x(), before.get_y()-center.get_y(), before.get_z()-center.get_z());		// Ortanca eklemin (0,0,0)'a taşındığı düşünülürse, önceki eklemin yeni konumu hesaplanır. 
	coordinate new_next = coordinate(next.get_x()-center.get_x(), next.get_y()-center.get_y(), next.get_z()-center.get_z());				// Ortanca eklemin (0,0,0)'a taşındığı düşünülürse, sonraki eklemin yeni konumu hesaplanır. (Vektör çarpımı yapmak için ortanca eklem (0,0,0) noktasına taşınmalı.)

	double cross_product_y = new_before.get_z()*new_next.get_x() - new_before.get_x()*new_next.get_z();										// AxB vektörünün Y bileşeni hesaplanır.

	if(((joint_1_angle < 90) && (joint_1_angle > -90)) && cross_product_y > 0)  my_angle = 360 - my_angle;									// Robot kol, +X yarım küresine bakıyorsa ve vektörel çarpımın Y bileşeni 0 dan büyükse, açı yanlıştır ve 360'dan çıkarılır.
	if(((joint_1_angle > 90) || (joint_1_angle < -90)) && cross_product_y < 0)  my_angle = 360 - my_angle;									// Robot kol, -X yarım küresine bakıyorsa ve vektörel çarpımın Y bileşeni 0 dan küçükse, açı yanlıştır ve 360'dan çıkarılır.

	return my_angle;																// Açı döndürülür.
}

/*Bu fonksiyon, rotasyon matrisi formülleri kullanarak 3 boyutlu uzaydaki bir noktayı XY düzlemine göre verilen açı kadar döndürür,
noktanın konumunu günceller. 1. eklem açısı hesaplandıktan sonra diğer eklemler bu açı kadar döndüğü için eklemlerin yeni konumlarını
bulmak için kullanılır.*/
// to_be_rotated: Döndürülecek noktanın referansı (Referans olduğu için yapılan değişim main() deki noktayı etkileyecektir.)
// angle: Noktanın döndürüleceği açı miktarı
// sin()/cos(): Sinüs ve kosinüs hesaplamak için kullanılan fonksiyonlar (<cmath> kütüphanesinde tanımlanmışlardır.).
...
/*Fabrik algoritması, verilen eklemlerin konumlarını, robot kolun uç noktası new_end_point_pos olacak şekilde yeniden hesaplar.
Algoritmanın çalışma mantığı için: https://drive.google.com/drive/folders/10Jx3R2WmaSRwj1mYTVzVz-0sIhBbL4JL*/
// my_joints: Eski eklem konumlarını içeren vektörün referansı (Referans olduğu için eklem konumları kolaylıkla güncellenebilir.).
// link_lengths: Link uzunluklarını tutan array'in ilk elemanının adresi
// new_end_point_pos: Robot kolun ucunun gitmesinin istendiği nokta
// REACH: Robot kolun tam açılımda uzanabildiği maksimum mesafe
