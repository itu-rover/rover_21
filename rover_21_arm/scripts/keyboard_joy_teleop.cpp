...
sadece 1 ve -1 kullanılarak işlem basitleştirilmiştir).
	12 adet buton için ise 1 seçeneği, klavyenin farklı tuşlarına atanmıştır (Butonlar, aktifleştirilmezse 0 değerini alırlar).
*/

void fill_message(sensor_msgs::Joy& joy_message, char keyboard_char){			

	if(keyboard_char == 'q')	joy_message.axes[0] = 1;
	else if(keyboard_char == 'w')	joy_message.axes[0] = -1;
	else if(keyboard_char == 'e')	joy_message.axes[1] = 1;
	else if(keyboard_char == 'r')	joy_message.axes[1] = -1;
	else if(keyboard_char == 't')	joy_message.axes[2] = 1;
	else if(keyboard_char == 'y')	joy_message.axes[2] = -1;
	else if(keyboard_char == 'o')	joy_message.axes[3] = 1;
	else if(keyboard_char == 'p')	joy_message.axes[3] = -1;
	else if(keyboard_char == 'a')	joy_message.axes[4] = 1;
	else if(keyboard_char == 's')	joy_message.axes[4] = -1;
	else if(keyboard_char == 'd')	joy_message.axes[5] = 1;
	else if(keyboard_char == 'f')	joy_message.axes[5] = -1;

	else if(keyboard_char == 'g')	joy_message.buttons[0] = 1;
	else if(keyboard_char == 'h')	joy_message.buttons[1] = 1;
	else if(keyboard_char == 'j')	joy_message.buttons[2] = 1;
	else if(keyboard_char == 'k')	joy_message.buttons[3] = 1;
	else if(keyboard_char == 'l')	joy_message.buttons[4] = 1;
	else if(keyboard_char == 'z')	joy_message.buttons[5] = 1;
	else if(keyboard_char == 'x')	joy_message.buttons[6] = 1;
	else if(keyboard_char == 'c')	joy_message.buttons[7] = 1;
	else if(keyboard_char == 'v')	joy_message.buttons[8] = 1;
	else if(keyboard_char == 'b')	joy_message.buttons[9] = 1;
	else if(keyboard_char == 'n')	joy_message.buttons[10] = 1;
	else if(keyboard_char == 'm')	joy_message.buttons[11] = 1;
	else return;
}
...
boş mesaj yayınını durdurmak ve dolu mesaj yayınlamaktır. 117,118,119,120 ve 128. satırlardaki kodlar, getchar() fonksiyonunu
karakter alırken enter tuşuna basılmasını beklemeyecek hale getiren OS yöntemlerinden oluşur.
*/
...
void* read_keyboard(void* ptr){
	...
	while(1){
		...
		*(my_info_str->ready_ptr) = false;											// main()' e boş mesaj basmaması gerektiği gönderilir (Çünkü klavye veri okumaya hazır değildir).

		joy_message = empty_joy_message;											// joy_message, doldurulmadan önce boş mesaj olmalıdır (Çünkü doldurulmadan önce boş olduğundan emin olunmalıdır).
		...
		tcgetattr( STDIN_FILENO, &oldt);         
  		...
  		*(my_info_str->ready_ptr) = true;											// Klavyenin okumaya hazır olduğu main()'e bildirilir ve main() boş mesaj basmaya devam eder

  		keyboard_char = getchar();  												// Klavyede bir tuşa basılana kadar thread bu satırda bekler ve main() fonksiyonu boş mesaj basmaya devam eder.

  		*(my_info_str->new_msg_ptr) = true;											// Klavyede bir tuşa basılınca dolu mesaj geleceği main()'e bildirilir ve main() boş mesaj yayınlamayı durdurur.
		...
		*(my_info_str->new_msg_ptr) = false;										// Dolu mesaj yayınlandıktan sonra main()'e boş mesaj yayınlayabileceği bildirilir. 
	}
...
int main(int argc, char* argv[]){
	...
	ros::Publisher joy_msg_pub = pub_handle.advertise<sensor_msgs::Joy>("/joy", 1000);		// /joy topic'ine veri basmak için publisher objesi oluşturulur.

	ros::Rate loop_rate(10);														// Loop rate'in 100 den çok daha düşük olması, basılan boş mesaj sayısını ve sıklığını azaltmak içindir.
	...
	pthread_t id;    
	pthread_create(&id, NULL, read_keyboard, (void*) &info_str);					// info_str'ın adresi thread'e geçirilir ve read_keyboard(), ek thread'de çalışmaya başlar.
